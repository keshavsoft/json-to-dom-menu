...
const renderMenu = ({ inMenu } = {}) => {
    ...
    if (hasBrand) {
        ...
        brandTextDiv.innerHTML = `
            <span class="fs-5 fw-bold lh-1">${brand.title || ""}</span>
            ${brand.subtitle ? `<small class="text-secondary" style="font-size: 0.72rem;">${brand.subtitle}</small>` : ""}
        `;

        brandAnchor.appendChild(brandTextDiv);
        topBarContainer.appendChild(brandAnchor);
    } else {
        topBarContainer.className = "container-fluid px-2 px-md-4 d-flex flex-wrap align-items-center justify-content-center position-relative";
    }
    ...
    hamburgerBtn.className = hasBrand
        ? "btn btn-outline-secondary text-white border-secondary d-md-none my-1 px-2 py-1"
        : "btn btn-outline-secondary text-white border-secondary d-md-none my-1 px-2 py-1 position-absolute end-0 me-3";
    ...
    const mobileNav = renderMobileNav({
        ...
        inMobileFlavor: localMenu.mobileFlavor,
        inOnAction: localMenu.handleItemClick.bind(localMenu)
    });
